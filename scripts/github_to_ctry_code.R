...
github_to_ctry_code <- function(df, location) {
  ...
  # first we have to recode all the strings that could arise in the location column as country codes
  ...
  am	<- "\\b(?i)(Armenia)\\b"
  aw	<- "\\b(?i)(Aruba)\\b"
  au	<- "\\b(?i)(Australia|Melbourne(?![, FL|, UK| UK])|Sydney(?!, Nova)|Wollongong|Brisbane|Canberra)\\b"
  ...
  kh	<- "\\b(?i)(Cambodia)\\b"
  cm	<- "\\b(?i)(Cameroon)\\b"
  ca  <- "\\b(?i)(Canada|Ontario(?!, CA)|Quebec|nova scotia|new brunswick(?![, N|,NJ| NJ])|manitoba|british columbia|, BC|prince edward island|saskatchewan|alberta|newfoundland|toronto|Vancouver(?![ WA| Washington| WA, USA|, WA, USA])|, ON|Winnipeg|, B.C.|,ON|(?<!Melbourne, )Victoria(?![ Australia|, Australia])|Burnaby|Fredericton|Montreal|Qubec)\\b"
  cv	<- "\\b(?i)(Cape Verde)\\b"
  ky	<- "\\b(?i)(Cayman Islands)\\b"
  cf	<- "\\b(?i)(Central African Republic)\\b"
  td	<- "\\b(?i)(Chad)\\b"
  cl	<- "\\b(?i)(Chile)\\b"
  cn  <- "\\b(?i)((?<!Hong Kong, )China|Beijing|GuangZhou|Shanghai|Tianjin|Shenzhen|Chengdu|Dongguan|Chongqing|Guangdong|ZhuHaiChina|Zhu haiChina|Zhengzhou|ZheJiang|HangZhou|zh-cn|(?<![Zurich|Lausanne]), CH|, CHN|Xiamen|Wuhan|beijingchina|Changsha|Hunan|ChengduChina|chinabeijing|ChinaShenzhen|^guang|^Hangz|Harbin Institute|Hefei|Hunan|^jiang|^Nanjing|^ShenZhen|GuangzhouChina|GuangZhouChina|NanjingChina|ShenzhenChina|XiamenChina|shenzhenChina|BeijingHaidian|wuhanChina)\\b"
  ...
  cd	<- "\\b(?i)(Democratic Republic of the Congo|Brazzaville|Brazzaville|Brazzaville|Brazzaville, Congo, republic of|Congo - Brazzaville)\\b"
  ...
  ge	<- "\\b(?i)(Tbilisi|tbiilisi|Batumi|Kutaisi)\\b" #"\\b(?i)((?<!, )Georgia(?!, U)|Tbilisi)\\b"
  ...
  kr	<- "\\b(?i)(South Korea|Seoul|Seuol|Daejeon|pangyo|Seongnam-si, Gyeonggi-do, Korea|Rep. of Korea|republic of korea|Korean)\\b"
  ...
  nr	<- "\\b(?i)(Nauru)\\b"
  np	<- "\\b(?i)(Nepal)\\b"
  nl <- "\\b(?i)(Netherlands|(?<!rue d')Amsterdam|Holland(?!, [MO])|Zwolle|Willemstad|Curacao|Curaao|Utrecht|Breda|Eindhoven|Groningen)\\b"
  ...
  pt <- "\\b(?i)(Portugal|Lisboa|Lisbon|Azores|Porto(?! [Alegre|Velho])|Faro)\\b"
  pr	<- "\\b(?i)(Puerto Rico)\\b"
  qa	<- "\\b(?i)(Qatar)\\b"
  re	<- "\\b(?i)(Réunion)\\b"
  ro <- "\\b(?i)(Romania|Bucharest)\\b"
  ru <- "\\b(?i)(Russia|Russian|Moscow(?!, I)|Yekaterinburg|St Petersburg(?![, F| FL| florida])|St. Petersburg(?![, F| FL| florida])|St-Petersburg(?!, F)|RU, |Simferopol|Sevastopol)\\b"
  rw	<- "\\b(?i)(Rwanda)\\b"
  bl	<- "\\b(?i)(Saint Barthélemy)\\b"
  sh	<- "\\b(?i)(Saint Helena)\\b"
  kn	<- "\\b(?i)(Saint Kitts|Nevis|Basseterre, St. Kitts)\\b"
  ...
  es <- "\\b(?i)(Spain|Madrid|Barcelona, Spain|Granada|Sevilla|Mallorca|Ibiza|Bilbao|Espana|Espaa|Zaragoza|Catalunya|Basque|Valencia(?!, [C|Ven])|Catalonia|Galicia)\\b"
  ...
  gb <- "\\b(?i)(UK|UK,|United Kingdom|united-kingdom|London(?![, Ontario|, ON|Ontario| Canada|, Canada|, CA])|(?<!New )England|Wales|U.K.|Aberdeen|Cardiff(?!, CA)|Scotland|Edinburgh|Glasglow|Brighton(?!, [M|C]))\\b"
  us	<- "\\b(?i)(?:Ala(?:(?:bam|sk)a)|American Samoa|Arizona|Arkansas|(?<!Baja )California|Colorado|Connecticut|Delaware|District of Columbia|Florida|(?<!Tbilisi, )Georgia(?!, tbilisi)|Guam|Hawaii|Idaho|Illinois|Indiana|Iowa|Kansas|Kentucky|Louisiana|Maine|Maryland|Massachusetts|Michigan|Minnesota|Miss(?:(?:issipp|our)i)|Montana|Nebraska|Nevada|New (?:Hampshire|Jersey|Mexico|York)|North (?:(?:Carolin|Dakot)a)|Ohio|Oklahoma|Oregon|Pennsylvania|Rhode Island|South (?:(?:Carolin|Dakot)a)|Tennessee|Texas|Utah|Vermont|Virgin(?:ia| Island(s?))|Washington|West Virginia|Wisconsin|Wyoming|United States|A[KLRSZ]|(?<![Ontario |Ontario, |Toronto |Toronto, |ON, |ON |Vancouver |Vancouver, |Vancouver,])C[AOT]|(?<![Berlin, |Hamburg, |Munich, |Dresden, |Frankfurt, ])DE|FL|G[AU]|HI|(?<![Jakarta, |Bandung, |Yogyakarta, |Pune, ])I[ALDN]|K[SY]|, LA|M[DEINOST]|(?<!Imperatriz-)MA|N[CDEHJMVY]|O[HK]|, OR|PA|RI|(?<!Blumenau, )S[CD](?![ - Brasil| - Brazil|, Brazil|, Brasil| Brazil| Brasil| - BR])|T[NX](?!, India)|UT|V[AIT]|W[AIVY](?!, Australia)|NYC|DC(?!, Colombia)|D.C.|US|USA|Seattle|San Francisco|Chicago|Philly|Philadelphia|Los Angeles|Portland|Houston|Phoenix|San Antonio|San Diego|Oakland|Boston|Bay Area|Berkeley|Stanford|Harvard|Dallas|Silicon|San Jose(?!, Costa Rica)|USC|Twin Cities|NYU|Yale|, Mass| Mass|newyork|new_york|U.S.|Atlanta|Austin|Albany|Manhattan|Brooklyn|Bronx|Cal|Cape Cod|Cleveland|Denver|Detroit|East Bay|Pittsburgh|Salt Lake|St. Louis|Honolulu|Miami|Minneapolis|Palo Alto|^san fran|socal|^SF|Merica$|Greater Nashville Area)\\b"
  uy	<- "\\b(?i)(Uruguay)\\b"
  uz	<- "\\b(?i)(Uzbekistan)\\b"
  vu	<- "\\b(?i)(Vanuatu)\\b"
  ve	<- "\\b(?i)(Venezuela)\\b"
  vn	<- "\\b(?i)(Viet Nam|Vietnam|Vit Nam)\\b"
  ...
  classified_data <- data.table(df) %>%
    ...
    dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = af), yes = "af", no = "")) %>%
    ...
    dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = "\\b(?i)(India|Mumbai|Delhi|Kolkata|Chennai|Bangalore|Hyderabad|Ahmedabad|Pune|VIT (?!Nam)|NIT |Amritsar|(?<!Lahore[ ,])Punjab(?! Pakistan)|Bhubaneswar|Gandhinagar|Gujarat|^IIT |Indian|Jaipur|jammu|kashmir|^MNNIT)\\b"), paste("in", detected_country, sep=","), no = detected_country)) %>%
  ...
  #classified_data$new_country_code <- gsub("\\,$", "", classified_data$new_country_code)
  #classified_data$new_country_code <- trimws(classified_data$new_country_code)

  #classified_data <- classified_data %>%
  #  select(login, new_country_code, everything()) %>%
  #  drop_na(new_country_code)
