...
CREATE MATERIALIZED VIEW gh_cost.cost_logins_w_sector_info AS (
...
GRANT ALL PRIVILEGES ON TABLE gh_cost.cost_logins_w_sector_info TO ncses_oss;

--get the counts
WITH new_table AS (
	SELECT COALESCE(sector, 'n/a') AS sector, login, commits, additions, deletions
	FROM gh_cost.cost_logins_w_sector_info
)
