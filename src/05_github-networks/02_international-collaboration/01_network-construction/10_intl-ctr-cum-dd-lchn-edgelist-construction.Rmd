...
sna_intl_ctr_nodelist <- data.frame(id = c(1:(igraph::vcount(intl_ctr_network))), 
                                    login = igraph::V(intl_ctr_network)$name)

# counting the number of isolates 
gorder(intl_ctr_network) 
gsize(intl_ctr_network)
sum(degree(simplify(intl_ctr_network))==0) # isolates
...
dbWriteTable(conn, name = c(schema = "gh_sna" , name = str_c("sna_intl_ctr_edgelist_dd_lchn_", analysis_year)), 
...
GRANT ALL PRIVILEGES ON gh_sna.sna_ctr_edgelist_dd_lchn_0813 TO ncses_oss;
