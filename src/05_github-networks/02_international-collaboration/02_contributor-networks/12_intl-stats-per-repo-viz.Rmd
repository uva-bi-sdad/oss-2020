...
```{sql}
CREATE MATERIALIZED VIEW gh.desc_repos_intl_summary AS (
...
repos_summary <- dbGetQuery(conn, "SELECT slug, logins, commits, additions, deletions  
                                   FROM gh.desc_repos_intl_summary")
...
treemap::treemap(df, 
        ...
        vSize="totals",
        palette = pal, 
        title="Number of contributors per repository (International Data, 2008-19)",
