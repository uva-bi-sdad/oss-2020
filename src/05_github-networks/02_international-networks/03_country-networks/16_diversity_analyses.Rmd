...
vis_miss(wdi_github_joined %>% 
           #filter(year == 2016) %>% 
           select(country_name, year, gdp_per_capita, servers_permil, prc_internet, prc_comp_exports, prc_comp_imports, high_tech_exports, 
                  trademark_apps, prc_rnd_expnd, rnd_rchrs_permil, rnd_techs_permil, gini_index, everything(), -users:-fstgrdy_range))
```
```{r}
# notes 
...
```{r}
test <- wdi_github_joined %>% 
  select(country_name, year, trademark_apps) %>% 
  drop_na(trademark_apps)
```

```{r}
filtered_df %>% 
  filter(grepl("Population estimates", indicator_name))
```
