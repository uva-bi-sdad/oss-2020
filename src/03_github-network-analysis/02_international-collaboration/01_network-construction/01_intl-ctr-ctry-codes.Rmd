...
# load packages 
for (pkg in c("tidyverse", "igraph", "data.table", "tidytable", "countrycode",
              "R.utils", "RPostgreSQL", "cowplot", "maditr")) {library(pkg, character.only = TRUE)}
...
oss_data <- dbGetQuery(conn, "SELECT login, country_code, location, city, long, lat, email, company FROM gh.ctrs_extra")
academic_data <- dbGetQuery(conn, "SELECT login, institution, country, is_academic FROM gh.sna_ctr_academic;")
city_codes <- dbGetQuery(conn, "SELECT * FROM gh.sna_ctr_city_codes;")
...
zw	<- "\\b(?i)(Zimbabwe)\\b"
asia <- "\\b(?i)(Asia)\\b"
africa <- "\\b(?i)((?<!South )Africa|Northern Africa|North Africa|West Africa|East Africa)\\b"
...
middle_east <- "\\b(?i)(EMEA|Middle East|eurasia)\\b"
planet_earth <- "\\b(?i)(Planet Earth|Worldwide|Earth planet|Earth, for now|Earth, Milky Way|Somewhere on Earth|Somewhere in the world|Earth|Earth, Solar System|Earth, Solar System, Milky Way|Anywhere, Earth|Somewhere in the world...|Somewhere on Planet Earth|Somewhere on the earth)\\b"
somewhere_in_space <- "\\b(?i)(deep space|Universe|Milky Way|Virgo Supercluster|ZZ9 Plural Z Alpha|Sector ZZ9 Plural Z Alpha|The Universe|milky way|Outer Space|3rd rock from the sun|The Milky Way|Galactic Sector ZZ9 Plural Z Alpha|Earth, Solar System, Milky Way, Laniakea|Laniakea Supercluster|Somewhere in the universe|Milkey Way|Middle-Earth|Milkyway Galaxy)\\b"
#some_any_nowhere <- "Somewhere|Nowhere|Anywhere|everywhere|Somewhere.|The middle of nowhere"
ficticious_places <- "\\b(?i)(Gensokyo|Matrix|The Matrix|Wonderland|Somewhere over the Rainbow|Somewhere under the rainbow|5th dimension|4th dimension)\\b"
worldwide_web <- "\\b(?i)(worldwide|international|WWW|cyberspace|Internets|Interwebz|interwebs)\\b"
...
users_gh_cc <- data.table(oss_data) %>%
  ...
  dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = middle_east), 
                                      paste("middle_east", detected_country, sep=","), no = detected_country)) %>%
  dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = planet_earth), 
                                      paste("planet_earth", detected_country, sep=","), no = detected_country)) %>%
  dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = somewhere_in_space), 
                                      paste("somewhere_in_space", detected_country, sep=","), no = detected_country)) %>%
  #dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = some_any_nowhere), 
  #                                    paste("some_any_nowhere", detected_country, sep=","), no = detected_country)) %>%
  dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = ficticious_places), 
                                      paste("ficticious_places", detected_country, sep=","), no = detected_country)) %>%
  dt_mutate(detected_country = ifelse(test = str_detect(string = oss_data$location, pattern = worldwide_web), 
                                      paste("worldwide_web", detected_country, sep=","), no = detected_country)) %>%
...
Ok, now we need to see if we can increase the number of users classified based on the other column data. 

First, let's try imputing old country_code information into new_county_code information, cleaning spaces that were misclassified, and renaming common multiple country mistakes. This cleaning step actually lowers the count because of false-positive spaces that were counted as a country code.  

```{r}
# after recoding all the strings into new_cc, lets impute old_cc into new_cc when missing 
users_with_old_cc <- as_tibble(users_gh_cc) %>% 
  select(login, new_country_code, country_code, everything()) %>% 
  mutate(new_country_code = ifelse(is.na(new_country_code), country_code, new_country_code),
         new_country_code = ifelse(new_country_code == "", country_code, new_country_code),
         country_code = trimws(country_code),
         new_country_code = trimws(new_country_code),
            # fixing systematic false positives (would be better if i did this in regex but it would take a long time)
         new_country_code = ifelse(test = str_detect(string = new_country_code, pattern = "us,br"), yes = "br", no = new_country_code),
         new_country_code = ifelse(test = str_detect(string = new_country_code, pattern = "us,in"), yes = "in", no = new_country_code))

users_with_old_cc %>% 
  drop_na(new_country_code)

multiple_cc_fixes <- users_with_old_cc %>% 
  dt_filter(grepl(",", new_country_code)) %>% 
  #separate_rows(new_country_code, sep = "," , convert = FALSE) %>% 
  mutate(check = str_replace_all(new_country_code, "planet_earth", ""),
         check = str_replace_all(check, "worldwide_web", ""),
         check = str_replace_all(check, "somewhere_in_space", ""),
         check = str_replace_all(check, "europe", ""),
         check = str_replace_all(check, "asia", ""),
         check = str_replace_all(check, "americas", ""),
         check = str_replace_all(check, "africa", ""),
         check = str_replace_all(check, "middle_east", ""),
         check = str_replace_all(check, "ficticious_places", ""),
         check = str_replace_all(check, "\\,$", ""),
         check = str_replace_all(check, "\\,$", ""),
         check = str_replace_all(check, "\\,$", "")) %>% 
  #mutate(check = ifelse(str_detect(new_country_code, "\\b(\\,)\\b"), "us", "bleh")) %>% 
  select(login, check) 

users_with_old_cc <- users_with_old_cc %>% 
  left_join(multiple_cc_fixes, by = "login") %>% 
  mutate(new_country_code = ifelse(is.na(check), new_country_code, check)) %>% 
  mutate(new_country_code = na_if(new_country_code, "")) %>% select(-check) 

users_with_old_cc %>% 
  drop_na(new_country_code)

# around 5,000 cases that do not match 
non_matching <- users_with_old_cc %>% 
  dt_mutate(match_test = ifelse(country_code == new_country_code, TRUE, FALSE)) %>% 
  dt_filter(match_test == FALSE) %>% 
  select(login, new_country_code, country_code, location, city, match_test, everything())
non_matching # there is some room to improve with these (might just impute original country code instead)

non_matching %>% 
  count(new_country_code) %>% 
  arrange(-n)

# dropped from 2,078 that do not match (bc we allow multiple countries) to ~540 after recoding 
non_matching %>% 
  select(login, country_code, new_country_code, everything()) %>% 
  dt_filter(grepl(",", new_country_code)) #%>% count(new_country_code) %>% arrange(-n)

```

Now, let's see if the sectored academic data (with country codes) help classify more users.

```{r}
# joining increases dataset from 717492 to 717558
users_with_uni_cc <- users_with_old_cc %>% 
  left_join(academic_data, by = "login") %>% 
  #select(login, country, country_code, new_country_code, institution) %>% 
  #filter(is.na(new_country_code) & !is.na(country)) %>% 
  mutate(tmp_cc = countrycode(country, origin = 'country.name', destination = 'iso2c'),
         tmp_cc = tolower(tmp_cc),
         new_country_code = ifelse(is.na(new_country_code), tmp_cc, new_country_code)) %>% 
  select(login, new_country_code, country_code, everything(), 
         -institution, -tmp_cc, is_academic, -country, -is_academic)
...
users_with_uni_cc %>% 
  drop_na(new_country_code)
# we are up from 700,105 users in the first draft of the Scientific Reports paper to 733,399 users 
```

Next, let's try emails. This entails pulling out all of the country codes from the email subdomains and then classifying into countries from there.

```{r}
email_domain_by_country <- read_csv("/sfs/qumulo/qhome/kb7hp/git/oss-2020/data/government/email_to_country_code.csv")
email_domain_by_country <- email_domain_by_country %>% 
  mutate(country_code = tolower(country_code)) %>% 
  rename(domain = country_email, 
         cc_by_email = country_code) %>% 
  select(domain, cc_by_email, country_name); email_domain_by_country

cc_by_email <- users_with_uni_cc %>% 
  filter(is.na(new_country_code) & !is.na(email)) %>% 
  dt_mutate(domain = str_extract(email, ".[^.]*$")) %>% 
  filter(domain != ".com" & domain != ".org" & domain != ".io" & domain != ".im" &
         domain != ".net" & domain != ".me" & domain != ".cc" & domain != ".co" & domain != ".sh") %>% 
  left_join(email_domain_by_country, by = "domain") %>% 
  drop_na(cc_by_email) %>% 
  select(login, cc_by_email) 

users_with_email_cc <- users_with_uni_cc %>% 
  left_join(cc_by_email, by = "login") %>% 
  mutate(new_country_code = ifelse(is.na(new_country_code), cc_by_email, new_country_code))

users_with_email_cc <- users_with_email_cc %>% 
  mutate(domain = str_extract(email, ".[^.]*$"), 
         alt_domains = ifelse(str_detect(domain, ".gov"), "us", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".mil"), "us", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".nyc"), "us", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".army"), "us", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".eu"), "europe", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".asia"), "asia", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".quebec"), "ca", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".berlin"), "de", new_country_code),
         alt_domains = ifelse(str_detect(domain, ".tokyo"), "jp", new_country_code),
         jp_cn = ifelse(str_detect(company, "china"), "cn", new_country_code),
         jp_cn = ifelse(str_detect(company, "japan"), "jp", new_country_code),
         new_country_code = ifelse(is.na(new_country_code), alt_domains, new_country_code),
         new_country_code = ifelse(is.na(new_country_code), jp_cn, new_country_code)) %>% 
  select(login, new_country_code, everything(), -cc_by_email, -alt_domains, -jp_cn)
...
users_with_email_cc %>% 
  drop_na(new_country_code)
# we are up from 700,105 users in the first draft of the Scientific Reports paper to 751,222 users 
```
...
users_with_email_cc %>% 
  ...
  left_join(email_domain_by_country, by = "domain") %>% 
  filter(domain == ".edu") %>% 
  select( email, company, login, everything())
# also about 550 .edu people that should be classified in academia
# most are cases where the put info in the company column but still have .edu domains 
# too time consuming to pursue this here, but could develop in academia 

# could improve on this -- about 30-40 rows of random cities without country codes
users_with_email_cc %>% 
  filter(is.na(new_country_code) & !is.na(city)) %>% 
  count(location) %>% 
  arrange(-n)

# also 200 rows with lat and lon but no country code  
users_with_email_cc %>% 
  filter(is.na(new_country_code) & !is.na(long) & long != 0) %>% 
  count(location) %>% 
  arrange(-n)
```
...
```{r}
users_with_countries <- users_with_email_cc

users_with_countries <- users_with_countries %>% 
  dplyr::mutate(new_country_code = trimws(new_country_code)) %>%
  drop_na(new_country_code) %>% 
...
users_with_countries$country_code <- stringi::stri_trim_both(users_with_countries$country_code)
users_with_countries$country_code_di <- stringi::stri_trim_both(users_with_countries$country_code_di)
users_with_countries$country_code_vis <- stringi::stri_trim_both(users_with_countries$country_code_vis)
...
```{r}
# unique users 
length(users_with_countries$login)
```
```{r}
...
```
```{r}
users_with_countries %>% dplyr::filter(grepl(",", country_code)) %>% count() / length(users_gh_cc$login)
```
```{r}
exclusion_codes <- c("europe", "asia", "africa", "americas", "middle_east", "planet_earth", 
                     "worldwide_web", "somewhere_in_space", "ficticious_places") 
users_with_countries %>% dplyr::filter(country_code %in% exclusion_codes) %>% count()
users_with_countries %>% dplyr::filter(country_code %in% exclusion_codes) %>% count() / length(users_gh_cc$login)
```
```{r}
all_excluded <- c("europe", "asia", "africa", "americas", "middle_east", "planet_earth", 
                     "worldwide_web", "somewhere_in_space", "ficticious_places")  
users_with_countries %>% dplyr::filter(country_code %in% all_excluded | grepl(",", country_code)) %>% count()
users_with_countries %>% dplyr::filter(country_code %in% all_excluded | grepl(",", country_code)) %>% count() / length(users_gh_cc$login)
```
```{r}
users_with_countries <- users_with_countries %>% 
  mutate(country_code_vis = na_if(country_code_vis, "europe"),
         country_code_vis = na_if(country_code_vis, "asia"),
         country_code_vis = na_if(country_code_vis, "africa"),
         country_code_vis = na_if(country_code_vis, "americas"),
         country_code_vis = na_if(country_code_vis, "middle_east"),
         country_code_vis = na_if(country_code_vis, "planet_earth"),
         country_code_vis = na_if(country_code_vis, "worldwide_web"),
         country_code_vis = na_if(country_code_vis, "somewhere_in_space"),
         country_code_vis = na_if(country_code_vis, "ficticious_places"))

users_with_countries <- users_with_countries %>% 
  mutate(country_name = countrycode(country_code_vis, origin = 'iso2c', destination = 'country.name'),
         country_name = ifelse(str_detect(country_code_vis, "xk"), "Kosovo", country_name),
         country_name = ifelse(str_detect(country_code_vis, "multiple"), "Multiple Countries", country_name))

users_with_countries
```

```{r}
intl_totals_multiples_separated <- users_with_countries %>% 
...
write_csv(intl_totals_multiples_separated, "/sfs/qumulo/qhome/kb7hp/git/oss-2020/data/intl_totals/intl_totals_multiples_separated.csv")

intl_totals_multiples_included <- users_with_countries %>%
  dplyr::filter(!is.na(country_code_vis)) %>% 
  dplyr::count(country_code_vis, sort = TRUE)
intl_totals_multiples_included

write_csv(intl_totals_multiples_included, "/sfs/qumulo/qhome/kb7hp/git/oss-2020/data/intl_totals/intl_totals_multiples_included.csv")
```
...
dbWriteTable(conn, c("gh", "sna_ctr_ctry_codes"), users_with_countries, row.names = FALSE)
