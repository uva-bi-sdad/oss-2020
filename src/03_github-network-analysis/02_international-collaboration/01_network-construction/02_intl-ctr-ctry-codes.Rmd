...
kr	<- "\\b(?i)(South Korea|Seoul|Seuol|Daejeon|pangyo|Seongnam-si, Gyeonggi-do, Korea|Rep. of Korea|republic of korea)\\b"
...
ru <- "\\b(?i)(Russia|Russian|Moscow(?!, I)|Yekaterinburg|St Petersburg(?![, F| FL| florida])|St. Petersburg(?![, F| FL| florida])|St-Petersburg(?!, F)|RU, )\\b"
rw	<- "\\b(?i)(Rwanda)\\b"
...
middle_east <- "EMEA|Middle East|eurasia"
planet_earth <- "Planet Earth|Worldwide|Earth planet|Earth, for now|Earth, Milky Way|Somewhere on Earth|Somewhere in the world|Earth (for now)|Earth, Solar System|Earth, Solar System, Milky Way|Anywhere, Earth|Somewhere in the world...|Somewhere on Planet Earth|Somewhere on the earth"
somewhere_in_space <- "deep space|Universe|Milky Way|Virgo Supercluster|ZZ9 Plural Z Alpha|Sector ZZ9 Plural Z Alpha|The Universe|milky way|Outer Space|3rd rock from the sun|The Milky Way|Galactic Sector ZZ9 Plural Z Alpha|Earth, Solar System, Milky Way, Laniakea|Earth (currently)|Earth (mostly)|Laniakea Supercluster|Somewhere in the universe|Milkey Way|Middle-Earth|Milkyway Galaxy"
#some_any_nowhere <- "Somewhere|Nowhere|Anywhere|everywhere|Somewhere.|The middle of nowhere"
ficticious_places <- "Gensokyo|Matrix|The Matrix|Wonderland|Somewhere over the Rainbow|Somewhere under the rainbow|5th dimension|4th dimension"
worldwide_web <- "worldwide|international|WWW|cyberspace|Internets|Interwebz|interwebs"
...
First, let's try imputing old country_code information into new_county_code information 

```{r}
# after recoding all the strings into new_cc, lets impute old_cc into new_cc when missing 
users_with_old_cc <- users_gh_cc %>% 
...
# dropped from 5,126 cases that do not match to 3,883 after reiterative coding 
...
# dropped from 2,078 that do not match (bc we allow multiple countries) to 687 after recoding 
non_matching %>% 
...
users_with_uni_cc <- users_with_old_cc %>% 
  ...
  select(login, new_country_code, country_code, everything(), 
         -institution, -tmp_cc, is_academic, -country, -match_test, -is_academic)
...
users_with_uni_cc %>% 
  drop_na(new_country_code)
# we are up from 700,105 users in the first draft of the Scientific Reports paper to 733,074 users 
```
...
china_japan <- users_with_uni_cc %>%
  filter(is.na(new_country_code) & (company == "japan" | company == "china")) %>% 
  mutate(jp_cn = ifelse(str_detect(company, "china"), "cn", new_country_code),
...
users_with_email_cc <- users_with_uni_cc %>% 
  ...
  mutate(new_country_code = ifelse(is.na(new_country_code), cc_by_email, new_country_code), 
         ...
         new_country_code = ifelse(str_detect(domain, ".gov"), "us", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".mil"), "us", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".nyc"), "us", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".army"), "us", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".eu"), "europe", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".asia"), "asia", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".quebec"), "ca", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".berlin"), "de", new_country_code),
         new_country_code = ifelse(str_detect(domain, ".tokyo"), "jp", new_country_code)) %>%
  left_join(china_japan, by = "login") %>% 
  mutate(new_country_code = ifelse(is.na(new_country_code), jp_cn, new_country_code)) %>% 
  select(login, new_country_code, everything(), -cc_by_email, -jp_cn)
...
users_with_email_cc %>% 
  drop_na(new_country_code)
# we are up from 700,105 users in the first draft of the Scientific Reports paper to 750,885 users 
```
...
# could improve on this -- about 55 rows of random cities without country codes
...
# also 387 rows with lat and lon but no country code  
```
...
```
```{r}
users_gh_cc %>% dplyr::filter(grepl(",", country_code)) %>% count() / length(users_gh_cc$login)
```
```{r}
continent_codes <- c("europe", "asia", "africa", "americas")
users_gh_cc %>% dplyr::filter(country_code %in% continent_codes) %>% count()
users_gh_cc %>% dplyr::filter(country_code %in% continent_codes) %>% count() / length(users_gh_cc$login)
```
```{r}
all_excluded <- c("europe", "asia", "africa", "americas")
users_gh_cc %>% dplyr::filter(country_code %in% all_excluded | grepl(",", country_code)) %>% count()
users_gh_cc %>% dplyr::filter(country_code %in% all_excluded | grepl(",", country_code)) %>% count() / length(users_gh_cc$login)
```
...
dbWriteTable(conn, c("gh", "sna_ctr_ctry_codes"), users_gh_cc)
