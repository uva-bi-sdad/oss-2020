...
ctr_edgelist <- dbGetQuery(conn, "SELECT ctr1, ctr2, year, repo_wts 
                                  FROM gh_sna.sna_ctr_edgelist_yxy_lchn
...
```{r}
intl_ctr_edgelist %>% filter(is.na(ctrycode2))
```
...
dbWriteTable(conn, name = c(schema = "gh_sna" , name = "sna_intl_ctry_edgelist_yxy_lchn"), 
