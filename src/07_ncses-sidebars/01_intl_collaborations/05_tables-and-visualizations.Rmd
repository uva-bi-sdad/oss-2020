...
collab_df <- cbind(country_names, collab_df) %>% select(-country) %>% rename(country = Country)

# need to figure this one out 
collab_df <- collab_df %>% 
  mutate(num_non_us = if_else(num_non_us == 25999896, num_foreign, num_non_us))

collab_df %>% arrange(prc_foreign)

```

Step 2: Write the data to CSV 

```{r}
output_df <- collab_df %>% 
  select(country, num_collabs, num_domestic, num_foreign, num_us, prc_domestic, prc_foreign, prc_us) %>% 
  rename(total_collabs = num_collabs, us_collabs = num_us, 
         forgn_collabs = num_foreign, dom_collabs = num_domestic, prc_frgn = prc_foreign) 

output_df %>% arrange(-total_collabs)

write_csv(output_df, str_c("~/git/oss-2020/data/intl-indicator-output/oss_intl_collaborations_",
                           analysis_year,"_tograph.csv"))
```
...
intl_collabs_figure1 <- collab_df %>% 
  ...
  mutate(country = as.factor(country), country = fct_relevel(country, 
                               top_15_by_commits[1], top_15_by_commits[2], top_15_by_commits[3],
                               top_15_by_commits[4], top_15_by_commits[5], top_15_by_commits[6],
                               top_15_by_commits[7], top_15_by_commits[8], top_15_by_commits[9],
                               top_15_by_commits[10], top_15_by_commits[11], top_15_by_commits[12],
                               top_15_by_commits[13], top_15_by_commits[14], top_15_by_commits[15])) %>%
  ggplot(., aes(fill=collaborations, y=count, x=country)) + 
    ...
    labs(title = str_c("International collaborations on GitHub, for top-15 countries by GitHub commits: 2008-", analysis_year), 
    ...
    scale_fill_manual(name = "", values = c("#EAAA31","#232D4B"),
...
intl_collabs_figure2 <- collab_df %>% 
  ...
  theme(#legend.position="bottom", 
        ...
        axis.title.x = element_blank(),
        axis.text.x = element_text(angle = 35, hjust = 0.9)) +
  labs(title = str_c("International Collaborations on GitHub for Top-15 Countries Based on Total Collaborations from 2008-", analysis_year), 
       x = "Country", y = "Number of GitHub Collaborations Per Country (in Millions)") +
   scale_y_continuous(breaks=c(0,500000,1000000,1500000,2000000,2500000), 
                     labels=c("0", "0.5", "1.0", "1.5", "2.0", "2.5")) +
  scale_fill_manual(name = "", values = c("#EAAA31","#232D4B", "#628ED8"), # uva version
...
Gizem wants this to match the NCSES indicator that shows the fractions based on the total number of repos 
