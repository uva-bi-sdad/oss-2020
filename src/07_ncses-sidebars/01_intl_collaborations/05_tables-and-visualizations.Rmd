...
output_df <- collab_df %>% 
  ...
  rename(ttl_collabs = num_collabs, us_collabs = num_us, 
         frgn_collabs = num_foreign, dom_collabs = num_domestic, prc_frgn = prc_foreign) 
...
intl_collabs_figure2 <- collab_df %>% 
  ...
  theme(#legend.position="bottom", 
        ...
        axis.title.x = element_blank(),
        axis.text.x = element_text(angle = 35, hjust = 0.9)) +
  labs(title = str_c("International collaborations on GitHub, for top-15 countries based on total collaborations in ", analysis_year), 
       x = "Country", y = "Number of GitHub Collaborations Per Country") +
  scale_y_continuous(breaks=c(0,10000000,20000000,30000000,40000000,50000000), 
                     labels=c("0", "10M", "20M", "30M", "40M", "50M")) +
