...
# load packages 
for (pkg in c("tidyverse", "igraph", "visNetwork", 
              "bc3net", "data.table", "R.utils", "RPostgreSQL",
              "cowplot", "maditr", "lubridate", "linkprediction", 
              "poweRlaw")) {library(pkg, character.only = TRUE)}

# connect to postgresql to get data (in rivanna)
conn <- dbConnect(drv = PostgreSQL(), dbname = "sdad_data", 
                  host = "sdad.policyx-analytics.net", port = 5436, 
...
bar_version1 <- ggplot(data=totals, aes(x=year, y=n)) +
  ...
  theme(plot.title = (element_text("Amount of Repositories Created per Year in Millions", 
                                   colour="#232D4B", hjust = .5, size = 15)),
        ...
        panel.grid.minor = element_line(size = 0.25, linetype = 'solid',
                                        colour = "white"),
        axis.text.x=element_text(colour="#232D4B", size = 10),
        axis.text.y=element_text(colour="#232D4B", size = 10),
        axis.title = element_text(colour="#232D4B", size = 15)) + 
  labs(title="Amount of Repositories Created per Year in Millions", x="Year", y = "Amount in Millions") +
