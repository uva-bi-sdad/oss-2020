...
raw_collab_matrix <- read.csv(str_c("~/git/oss-2020/data/intl-indicator-output/oss_intl_collaborations_",
...
write_csv(collab_df, "~/git/oss-2020/data/intl-indicator-output/oss_intl_collaborations_2018_summary.csv")
...
intl_collabs_figure2 <- collab_df %>% 
  ...
  pivot_longer(!country, names_to = "collaborations", values_to = "count") %>% 
  mutate(country = as.factor(country), country = fct_relevel(country, 
                               "United States", "Germany", "United Kingdom", "China", "Canada", 
                               "France", "Netherlands", "Australia", "Sweden", "Japan",
                               "India", "Switzerland", "Spain", "Russia", "Brazil")) %>% 
...
```{r}
collab_df %>% arrange(-num_collabs)
```

```{r, fig.width=10, fig.height=5}
top_15_by_collabs <- c("United States", "India", "Germany", "China","United Kingdom", 
                       "Canada", "France", "Brazil", "Australia", "Japan", "Russia",
                       "Netherlands", "Poland", "Sweden", "Spain")
...
intl_collabs_figure2 <- collab_df %>% 
  ...
  pivot_longer(!country, names_to = "collaborations", values_to = "count") %>% 
  mutate(country = as.factor(country), country = fct_relevel(country, 
                               "United States", "India", "Germany", "China", "United Kingdom", 
                               "Canada", "France", "Brazil", "Australia", "Japan", "Russia", 
                               "Netherlands", "Poland", "Sweden", "Spain")) %>% 
  filter(!(country == "United States" & collaborations == "num_us")) %>% 
  ggplot(., aes(fill=collaborations, y=count, x=country)) + 
  ...
  theme(#legend.position="bottom", 
        ...
        axis.title.x = element_blank(),
        axis.text.x = element_text(angle = 35, hjust = 0.9)) +
  labs(title = str_c("International collaborations on GitHub, for top-15 countries based on GitHub collaborations: ", analysis_year), 
       x = "Country", y = "Number of GitHub Collaborations Per Country") +
  scale_y_continuous(breaks=c(0,10000000,20000000,30000000,40000000,50000000), 
                     labels=c("0", "10M", "20M", "30M", "40M", "50M")) +
  scale_fill_manual(#name = "", values = c("#E57200","#232D4B", "#628ED8"), # uva version
