...
# fix Ascension Island, Czechoslovakia, East Germany, European Union, Netherlands Antilles, SFR Yugoslavia, FR Yugoslavia
```
...
all_lists <- all_lists %>% 
  ...
  dt_mutate(institution = ifelse(test = str_detect(string = institution, 
                                 pattern = "\\b(?i)(global media, agency for)\\b"), 
                                 yes = "agency for global media", no = institution)) %>%
  dt_mutate(institution = ifelse(test = str_detect(string = institution, 
  ...
  dt_mutate(institution = ifelse(test = str_detect(string = institution, 
                                 pattern = "\\b(?i)(john f. kennedy center for performing arts)\\b"), 
...
all_lists %>% 
  ...
  grepl("office", institution))
 
# next steps 
# recode all of the crowdsourced data to match institutions 

#all_lists %>% filter(dataset == "az_agency")

all_lists %>% 
  filter(dataset == "az_agency" )  #) %>% filter(
  grepl("division", institution))
  
# human development services??, office of inspector general??, mint 
