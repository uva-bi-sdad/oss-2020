...
academic <- dbGetQuery(conn, "SELECT login, is_academic FROM gh.sna_ctr_academic;")
...
new_gov_string <- "\\b(?i)(university of)\\b"

new_academic_string <- "\\b(?i)(frederick national laboratory for cancer research)\\b"

combined_user_data %>% 
  count(is_gov)
  mutate(is_gov = ifelse(test = str_detect(string = is_gov, pattern = new_gov_string), yes = 1, no = is_gov)) %>% 
  mutate(is_gov = ifelse(test = str_detect(string = is_gov, pattern = new_gov_string), yes = 1, no = is_gov))
...
#  filter(is_gov == 1 & sector != "government") %>% 
#  filter(!grepl("university|berkeley|georgia tech|rochester institute of technology|california institute of technology", company))

classified_users %>% 
  filter(sector == "not classified") %>% 
  count(company) %>% 
  arrange(-n)
# next create is_missing column 
# add in international column 
# join the github commit data 
```
