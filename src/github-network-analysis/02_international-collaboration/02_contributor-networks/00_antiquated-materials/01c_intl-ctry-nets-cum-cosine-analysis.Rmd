...
# load packages 
for (pkg in c("tidyverse", "data.table", "R.utils", "RPostgreSQL", "igraph", "netrankr", "centiserve", "expm", "lsa",
...
ctry_cosine_edgelist <- reshape2::melt(cosine(as.matrix(as_adjacency_matrix(ctry_network, attr = "weight")), y = NULL))

# removing all of the edge weights below 0.01 as wagner et al. 2015 did 
ctry_cosine_edgelist <- ctry_cosine_edgelist %>% 
...
ctry_network <- simplify(graph.data.frame(ctry_cosine_edgelist, directed = FALSE), 
                         remove.loops = FALSE, 
                         edge.attr.comb = igraph_opt("edge.attr.comb"))
is_weighted(ctry_network)
...
network_stats$diameter <- diameter(ctry_network,directed=FALSE,unconnected=FALSE,weights=NULL)
...
setwd("~/oss-data/intl-ctry-nets-cum-cosine")
...
# Constructing Cosine Similarity Index

```{r}
library(lsa)
ctry_matrix <- as.matrix(get.adjacency(ctry_network))
cosine_matrix <- cosine(ctry_matrix, y = NULL) 
ctry_cosine_edgelist <- get.data.frame(graph.adjacency(cosine_matrix, weighted = TRUE))
ctry_cosine_edgelist %>% 
  filter(from != to) %>% 
  arrange(-weight)
```

# Constructing Jaccard Similarity Index 
