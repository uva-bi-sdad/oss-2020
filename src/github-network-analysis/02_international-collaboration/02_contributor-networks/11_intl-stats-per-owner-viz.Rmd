...
CREATE MATERIALIZED VIEW gh.desc_owners_intl_summary AS (
...
FROM (SELECT slug, sna_intl_ctr_nodelist.login, EXTRACT(YEAR FROM committed_date)::int AS YEAR, sna_intl_ctr_nodelist.country_code_vis AS country_code, additions, deletions
      FROM gh.commits_raw
      FULL JOIN gh.sna_intl_ctr_nodelist
      ON commits_raw.login = sna_intl_ctr_nodelist.login
      WHERE sna_intl_ctr_nodelist.country_code IS NOT NULL) A
GROUP BY owner
); 
...
owners_summary <- dbGetQuery(conn, "SELECT owner, repos, commits, additions, deletions  
                                  FROM gh.desc_owners_intl_summary")
