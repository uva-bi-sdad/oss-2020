...
dbDisconnect(conn)
```
## International Nodelist Creation 

And to make the intl_nodelist for the intl_edgelist we can just filter down the full sna_edgelist we made in 01_full-network-construction:
...
If we wanted to make dynamic nodelists that go year-by-year then we can use pull from our existing nodelist that has years:

```{sql write_sna_intl_ctr_nodelist_year}

---NEED TO TEST THIS TO MAKE SURE IT WORKS 
CREATE MATERIALIZED VIEW gh.sna_intl_ctr_nodelist_year AS (
SELECT login, year, repos, commits, deletions, country_code, country_code_di, country_code_vis 
FROM gh.sna_ctr_nodelist_full
WHERE country_code IS NOT NULL AND year = '2008' 
); 
