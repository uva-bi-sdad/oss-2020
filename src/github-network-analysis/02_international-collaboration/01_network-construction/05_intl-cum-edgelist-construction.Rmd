...
intl_ctr_edgelist <- ctr_edgelist %>% 
  filter(ctr1 != ctr2) %>% # removes all of the self-loops between individuals 
  rename(login = ctr1) %>% 
...
sna_intl_ctry_edgelist <- intl_ctr_edgelist %>% 
  select(country1, country2, ctrycode1, ctrycode2, repo_wts) %>% 
...
sna_intl_ctry_edgelist_test <- as_tibble(sna_intl_ctry_edgelist) %>% 
  rename(weight = repo_wts) %>% 
  select(ctrycode1, ctrycode2, weight)
...
dbWriteTable(conn, name = c(schema = "gh" , name = "sna_intl_ctry_edgelist_0819"), 
