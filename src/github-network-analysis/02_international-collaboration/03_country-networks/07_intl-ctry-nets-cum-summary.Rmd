...
for (pkg in c("tidyverse", "igraph", "visNetwork", "data.table", "extrafont",
              ...
              "poweRlaw", "plotly","gridExtra", "rmarkdown", 
              "grid")) {library(pkg, character.only = TRUE)}
...
```{r, fig.width=11.5, fig.height=3.75, echo=FALSE}
network_nodes <- global_netstats_cum %>% 
  ggplot( aes(time, node_count), y=node_count) +
...
grid.arrange(density_cum, dist_cum, transitivity_cum, fstgrdy_cum_logged, nrow=1, ncol=4)
```
```{r, fig.width=11, fig.height=2.2, echo=FALSE, layout="l-body-outset"}
...
centr_deg<-ggplot(global_netstats_cum) + 
  ...
  labs(y = "centr_deg", x = "Year", colour = "#D96403") + 
  theme_minimal() + theme(axis.title.x = element_blank()) +
  scale_x_discrete(breaks = c(2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019), 
                     labels = c(2008,"","","","",2013,"","","","","",2019))
centr_btw<-ggplot(global_netstats_cum) + 
  geom_line(aes(x=time, y=centr_btw, group=1), size=1.5) + 
  labs(y = "centr_btw", x = "Year", colour = "#D96403") + 
  theme_minimal() + theme(axis.title.x = element_blank()) +
  scale_x_discrete(breaks = c(2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019), 
                     labels = c(2008,"","","","",2013,"","","","","",2019))
centr_clo<-ggplot(global_netstats_cum) + 
  geom_line(aes(x=time, y=centr_clo, group=1), size=1.5) + 
  labs(y = "centr_clo", x = "Year", colour = "#600080") + 
  theme_minimal() + theme(axis.title.x = element_blank()) +
  scale_x_discrete(breaks = c(2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019), 
                     labels = c(2008,"","","","",2013,"","","","","",2019))
centr_eigen<-ggplot(global_netstats_cum) + 
  geom_line(aes(x=time, y=centr_eigen, group=1), size=1.5) + 
  labs(y = "centr_eigen", x = "Year", colour = "#600080") + 
  theme_minimal() + theme(axis.title.x = element_blank()) +
  scale_x_discrete(breaks = c(2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019), 
                     labels = c(2008,"","","","",2013,"","","","","",2019))

grid.arrange(centr_deg, centr_btw, centr_clo, centr_eigen, nrow=1, ncol=4)
```
