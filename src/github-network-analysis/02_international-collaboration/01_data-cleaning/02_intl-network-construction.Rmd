...
To make an international edgelist, we need first make sure that our full edgelist and the country_code data is indexed and then filter down the intl_edgelist for cases where both ctr1 + ctr2 columns actually have a valid country_code. 
...
CREATE INDEX login_cc_login_idx ON gh.login_ctry_codes (login);
CREATE INDEX login_cc_cc_idx ON gh.login_ctry_codes (country_code);

```

```{sql write_sna_intl_ctr_edgelist}
