...
dbWriteTable(conn, c("gh", "sna_intl_ctr_nodelist"), users_gh_cc)
...
CREATE INDEX sna_intl_ctr_nodelist_login_idx ON gh.sna_intl_ctr_nodelist (login);
CREATE INDEX sna_intl_ctr_nodelist_cc_idx ON gh.sna_intl_ctr_nodelist (country_code);
...
INNER JOIN gh.sna_intl_ctr_nodelist AS cc ON cc.login = ctr_el.ctr1 OR cc.login = ctr_el.ctr2 
WHERE cc.country_code IS NOT NULL AND cc.country_code IS NOT NULL
); 

```

I THINK SOME OF THIS MIGHT BE OUTDATED BUT I AM GOING TO LEAVE IT HERE FOR NOW  ########################

The last thing we need to do is create a dynamic bipartitle edgelist from our original commit_data. This will take two steps:

First, we need to bind the commit_data to the users with valid country_codes. This materializes a new commit_data_cc table.
...
The second step is creating a weighted edgelist of logins & repos with the weight for each year. This creates a new table called intl_dyn_bp_edgelist.

```{sql, echo=FALSE}
--code is outdated 
create table github.sna_bp_edgelist_cum_intl as (
