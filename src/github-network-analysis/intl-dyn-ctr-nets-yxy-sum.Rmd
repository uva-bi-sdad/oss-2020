...
global_netstats_yxy$time <- c("2008","2009","2010","2011","2012",
                              "2013","2014","2015","2016","2017","2018")
global_netstats_yxy <- global_netstats_yxy %>% select(time, everything())

# get k_core data from nodelist
k08<-full_nodelist_yxy %>% drop_na(kcore08) %>% 
  count(kcore08) %>% rename(core = kcore08, k08 = n)
k09<-full_nodelist_yxy %>% drop_na(kcore0809) %>% 
  count(kcore0809) %>% rename(core = kcore0809, k09 = n)
k10<-full_nodelist_yxy %>% drop_na(kcore0810) %>% 
  count(kcore0810) %>% rename(core = kcore0810, k10 = n)
k11<-full_nodelist_yxy %>% drop_na(kcore0811) %>% 
  count(kcore0811) %>% rename(core = kcore0811, k11 = n)
k12<-full_nodelist_yxy %>% drop_na(kcore0812) %>% 
  count(kcore0812) %>% rename(core = kcore0812, k12 = n)
k13<-full_nodelist_yxy %>% drop_na(kcore0813) %>% 
  count(kcore0813) %>% rename(core = kcore0813, k13 = n)
k14<-full_nodelist_yxy %>% drop_na(kcore0814) %>% 
  count(kcore0814) %>% rename(core = kcore0814, k14 = n)
k15<-full_nodelist_yxy %>% drop_na(kcore0815) %>% 
  count(kcore0815) %>% rename(core = kcore0815, k15 = n)
k16<-full_nodelist_yxy %>% drop_na(kcore0816) %>% 
  count(kcore0816) %>% rename(core = kcore0816, k16 = n)
k17<-full_nodelist_yxy %>% drop_na(kcore0817) %>% 
  count(kcore0817) %>% rename(core = kcore0817, k17 = n)
k18<-full_nodelist_yxy %>% drop_na(kcore0818) %>% 
...
global_netstats_cum <- right_join(global_netstats_cum,kmax_df, by = "time"); rm(kmax_df)
...
global_netstats_yxy <- global_netstats_yxy %>% 
  ...
  select(time, year, node_count, nodes_pctcng, edge_count, edges_pctcng, 
         #wtd_edge_count, wtd_edges_pctcng, 
         commits, commits_pctcng, 
...
kcore_graph <- kcore_data %>% 
  drop_na() %>%
  ggplot(aes(core)) + 
  geom_point(aes(y = k08, colour = "k08", group= 1), size=0.5) + 
  geom_point(aes(y = k09, colour = "k09", group= 2), size=0.5) +
  geom_point(aes(y = k10, colour = "k10", group= 3), size=0.5) +
  geom_point(aes(y = k11, colour = "k11", group= 4), size=0.5) +
  geom_point(aes(y = k12, colour = "k12", group= 5), size=0.5) +
  geom_point(aes(y = k13, colour = "k13", group= 6), size=0.5) +
  geom_point(aes(y = k14, colour = "k14", group= 7), size=0.5) +
  geom_point(aes(y = k15, colour = "k15", group= 8), size=0.5) +
  geom_point(aes(y = k16, colour = "k16", group= 9), size=0.5) +
  geom_point(aes(y = k17, colour = "k17", group= 10), size=0.5) +
  geom_point(aes(y = k18, colour = "k18", group= 11), size=0.5) +
